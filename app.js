app = (() => {
    ...
    const drawing=[{"type":"compound","data":[{"type":"closedbspline","data":[[635.3121789037599,540.4279297559557,
        678.4118214131591,538.2778763044897,721.2664244874752,532.7549799412452,773.1219507685297,530.7442983604565,
        820.0297644251376,522.5178969568357,819.7806085036896,522.5591148199925,819.5314525822414,522.6003326831493,
        793.3665734779188,597.069321110117,763.3812791131556,667.8200982398678,733.3311144450574,638.3789617974456,
        702.2726348043087,607.9755377957481,667.1276303390468,572.3244113175283,631.3721405028796,540.518393853207,
        631.5323266590669,540.6633659647944,631.6925128152542,540.8083380763816,632.2217162418151,540.4555115478871,
        632.7509196683764,540.1026850193927,633.263838391853,540.4787929945811,633.7767571153298,540.8549009697697,
        634.0629420187495,540.3969272729578,634.3491269221691,539.938953576146,634.8306529129643,540.1834416660508,
        635.3121789037599,540.4279297559557]]},{"type":"bspline","data":[417.53495909277564,550.7700476987712,
            429.2787219953061,544.5237544268924,441.54441671668184,541.0794832106643,467.3015793037032,538.8948508872429,
            494.34834001958495,541.2888081624917,545.8190674183176,544.270464109713,601.5659854060714,541.9655305303725,
            661.7410693777025,540.4263231266318,721.2664244874752,532.7549799412452,779.540452457873,530.4954229936715,
            831.5661433490224,520.3861957860821,858.320372600979,517.9989085448574,882.7750639668951,514.0653694285623,
            902.716030438746,510.9582001777255,917.2027355719114,507.36592222889095,930.9126430149526,504.0982671163057,
            940.103743247939,500.4697665603184,945.9057119286892,497.28890825708896,954.1815396278864,497.1126449126575]},
            {"type":"bspline","data":[875.8258015604764,369.9598761727264,873.2850477076283,371.20311674951915,
                870.9816091227849,375.4577376251698,866.0553154588146,387.1141811671964,859.3048046245696,402.9766370786871,
                846.7076950118807,435.9344250944848,835.9977635278137,474.2033799791516,801.0774956405168,580.3392047984989,
                758.0424424297943,680.3211675142938,744.1222869617894,713.5318901424245,729.5865685314463,742.4666266544524,
                719.7468955496638,759.6451649350936,710.3621189626307,771.7476450706006,704.1244410504364,780.4369812959453,
                701.0079666424488,784.7783554979942,699.4342161045196,786.9706532971718,698.45683419149,788.332185614556]},
                {"type":"bspline","data":[584.5392804119152,501.9194490949869,578.800844769155,497.80008803781107,
                    586.9085004896231,503.62020406229266,600.9155667724274,512.6591153710762,617.0251002937739,527.9321224373756,
                    660.0886255673563,565.1840380777304,702.2726348043087,607.9755377957481,778.884617358057,682.9716939545399,
                    852.9635657200619,755.6409388129009,878.3128177649523,778.4471356098388,900.7435638247465,801.6078908298323,
                    914.6661273931479,815.5211130936436,924.7552010613765,826.9225068705184,928.6769721066942,832.0565287451883,
                    930.9215276242221,835.8366743532237]}]}];

    const saved = [];
    function saveForUndo() {
        let s=JSON.stringify(drawing);
        while(true) {
            let idx=s.indexOf(',"cache":[[');
            if (idx==-1) break;
            let idx2=s.indexOf("]]",idx+10);
            s=s.substring(0,idx)+s.substring(idx2+2);
        }
        saved.push(s);
        if (saved.length > 1 && saved[saved.length - 1] == saved[saved.length - 2]) saved.pop();
    }
    let struct;
    let noTool = {
        mousedown() { },
    ...
    document.getElementById("undo").addEventListener("click", () => {
        while (saved.length > 0) {
            let old = saved.pop();
            let cur = JSON.stringify(drawing);
            if (old == cur) {
                continue;
            }
            drawing.splice(0, drawing.length);
            drawing.push.apply(drawing, JSON.parse(old));
            repaint();
            break;
        }
    });
